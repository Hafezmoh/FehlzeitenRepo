...
class dbmodel extends CI_Model
{
    ...
    function add_new_mitarbeiter($mit_name, $mit_benuzter, $mit_password, $mit_abteilung, $mit_is_admin)
    {
        $this->db->select('*');
        ...
        if (empty($result_array)) {
            $data = array(
                'm_name' => $mit_name, 'm_benutzer' => $mit_benuzter, 'm_passwort' => $mit_password,
                'm_abteillung_id' => $mit_abteilung, 'm_rule' => $mit_is_admin,
            );
            $this->db->insert('tbl_mitarbeiter', $data);
            return 1;
        } else {
            return 0;
        }
